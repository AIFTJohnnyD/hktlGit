...
const CompanyForm: FC<Record<string, any>> = () => {
  ...
  const onFinish = async (values: Record<string, any>) => {
    console.log('values', values); //values 就是页面中被填入的数据
    run(values);
    history.push('/application/loan-application-list-borrower');
  };

  const { data, error, loading } = useRequest(() => {
    return request('/api/borrower/get_borrower_from_id');
  });
  ...
  return (
    <PageContainer content="">
      <Card bordered={false}>
        <ProForm formRef={formRef} layout="vertical" onFinish={onFinish}>
          <Card
          ...
          >
            <Row gutter={16}>
              ...
              <Col xl={{ span: 6, offset: 2 }} lg={{ span: 6 }} md={{ span: 12 }} sm={24}></Col>

              <Col xl={{ span: 6, offset: 2 }} lg={{ span: 6 }} md={{ span: 24 }} sm={24}></Col>
            </Row>
          ...
          <Card
            title={<FormattedMessage id="pages.util.other" />}
            className={styles.card}
            bordered={false}
          >
            <Row gutter={16}>
