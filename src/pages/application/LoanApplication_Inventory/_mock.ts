...
import type { TableListItem, TableListParams, TableListItemLoan } from './data.d';
...
function postProduct(req: Request, res: Response, u: string, b: Request) {
  ...
  const { key, company_id, platform, product_id_in_platform,
    ...
    case 'PUT':
      (() => {
        ...
        tableListDataSource = tableListDataSource.map((item) => {
          if (item.key === key) {
            newProduct = { ...item, company_id, platform, product_id_in_platform,
              url, title, description,  
              category_0, category_1, category_2, category_3, category_4, category_5,
              list_price, discount_price, currency,
              growth_percentage, profit_margin, product_life_cycle,
              quantity_in_list_price, quantity_in_discount_price,
              status, remark };
            return { ...item, company_id, platform, product_id_in_platform,
              url, title, description,  
              category_0, category_1, category_2, category_3, category_4, category_5,
              list_price, discount_price, currency,
              growth_percentage, profit_margin, product_life_cycle,
...
function getListLoan(req: Request, res: Response) {
  const tableListDataSourceLoan: TableListItemLoan[] = [];

  for (let i = 0; i < 5; i += 1) {
    tableListDataSourceLoan.push({
      key: i,
      loan_name: 'Loan No. 88',
      loan_currency: 'HKD',
      ...
      other_cost_type: 'FIXED COST',
      other_cost_amount: 10 * i,    
  
      loan_start_date: '2021-01-01',
      loan_end_date: '2022-01-01',  
    });
  }
  
  res.json({ data: tableListDataSourceLoan});
}

export default {
  'GET /api/application': getProduct,
  'POST /api/application': postProduct,
  'PUT /api/application': postProduct,

  'GET /api/application/listloan': getListLoan,
};
